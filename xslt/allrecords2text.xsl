...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <xsl:text>DOI Last Updated</xsl:text>|<xsl:text>Series</xsl:text>|<xsl:text>Year Published</xsl:text>|<xsl:text>Month Published</xsl:text>|<xsl:text>Title</xsl:text>|<xsl:text>Authors</xsl:text>|<xsl:text>OU</xsl:text>|<xsl:text>DOI</xsl:text>|<xsl:text>URL</xsl:text>
    ...
    <xsl:template match="query">
        ...
        <xsl:value-of select="crm-item[8]"/>|<xsl:value-of select="@key"/>|<xsl:value-of
